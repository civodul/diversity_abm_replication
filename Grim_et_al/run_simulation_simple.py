...
fixed_params = {"n": 2000, "k": 3, "N_agents": 10, "l": 12, "strategy": "relay"}
variable_params = {"smoothness": list(range(21))}
...
if not "AM_I_IN_A_DOCKER_CONTAINER" in os.environ:
    out.to_pickle(
        "Grimmodel_results_" + datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + ".pkl"
    )
